...
nuget restore -Verbosity detailed -DisableParallelProcessing main/Main.sln
...
script -c "make XBUILD_VERBOSITY=diag V=1" build.log
...
#MSBuildExtensionsPath="$MONO_ANDROID_PATH/lib/xbuild" script -c "make V=1 XBUILD=xbuild" build.log
#MSBuildExtensionsPath="$MONO_ANDROID_PATH/lib/xbuild"
script -c "make V=1" build.log
